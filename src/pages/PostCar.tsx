...
const getUrlFromFile = async (myFile :any)=> {
  try {
    const imgData = await axios.post(cloudinaryApi, {file: myFile, upload_preset: upr})
    if((!imgData.data.secure_url)) throw new Error("cloud upload failed" )
...
const PostCar = () => {
  ...
  const [file, setFile] = React.useState<Partial<{name:string, type:string, size: number}>>();
  ...
  const validate =()=>{
    ...
    if(errFile.length > 0)  {setFormErr({...formErr, imageUrl: "check if your image is valid"})};
  }
      ...
      <div className="container d-flex mt-1 mx-3 flex-column align-items-center">
        ...
        <form onSubmit={handleSubmit} className="d-flex flex-wrap  rounded-alt bg-lighter col-12 col-lg-8 p-3 pt-4 align-items-center">
          ...
          <label htmlFor="image" id="upload-box" className="d-flex flex-column col-12 align-self-center align-items-center">
            ...
            <input type="file" id="image" className="rounded col-7 col-sm-6 border-light mt-3 border-1 " accept="image/jpg, image/jpeg, image/png" onChange={handleUpload} />
            {errFile && (
                <small className="mb-1 mt-2 text-danger">{errFile}, please check your file </small>)
            }
            {file &&(
              <span>
          ...
          <label htmlFor="fuelCapacity" className="col-10 col-md-5 mt-4 mx-4">
            ...
            <input type="number" value={formData.fuelCapacity ?? ''} onChange={handleChange} className="rounded border-light border-1 p-1 ps-2 w-100" placeholder="fuel tank's full capacity..." name="fuelCapacity" id="fuelCapacity" />
          </label>
